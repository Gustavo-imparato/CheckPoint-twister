  ...
  switch (stateGame) {
    ...
    case LEVEL_4:
      ...
      if (sequenciaJogador.equals(sequenciaNumerica)) {
        ...
        Serial.println("Parabens! Voce classificou para o level infinito!");
        tocarSomAcerto(); // Toca som de acerto
        piscaled(1000, 3);
      } else {
        stateGame = GAME_OVER;
      ...
      break;
    case LEVEL_INFINITO:
    Serial.println("* Nivel Infinito *" );
    
    // Gera a sequência com um número crescente de LED
    geraSequencia(600, ledsInfinitos); 

    if (jogarComBotoes) {
        recebeInputDoJogador(ledsInfinitos); 
    } else {
        recebeInputDoJogadorConsole(ledsInfinitos); 
    }

    if (sequenciaJogador.equals(sequenciaNumerica)) {
        ledsInfinitos++; // Aumenta a dificuldade
        Serial.println("Parabens! Continue jogando.");
        tocarSomAcerto(); 
        piscaled(1000, 3);
        Serial.println("Level" + contador);
        contador ++;
    } else {
        stateGame = GAME_OVER;
        Serial.println("Errooooo!!!"); 
    ...
    case PLAY_AGAIN:
      ...
      stateGame = LEVEL_1;
      Serial.println("Jogo recomeCando...");
      piscaled(1000, 3);
      break;
  }
}
